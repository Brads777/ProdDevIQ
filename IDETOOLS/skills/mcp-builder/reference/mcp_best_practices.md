...
Every tool definition your server exposes consumes tokens in the LLM's context window on every inference turn. Minimizing this footprint is critical for cost and quality.

See [Context Optimization Guide](./context_optimization.md) for comprehensive strategies.

### Token Budget Rules

1. **Target <100 tokens per tool description.** Write concise descriptions — lists over prose.
2. **Target <5,000 tokens total** for your entire server's tool definitions.
3. **Measure your footprint** before shipping. Count tokens with tiktoken or Anthropic's tokenizer.
4. **Consolidate similar tools** into parameterized functions. Three search tools → one `web_search(provider)` tool saves ~60% overhead.
5. **Put detailed docs in resources, not descriptions.** Tool descriptions are injected into every turn. Resources are loaded on demand.

### Tool Description Anti-Patterns

**Too verbose** (~45 tokens):
```
"This tool allows you to search for issues across all repositories in a
GitHub organization. It supports filtering by state (open, closed, all),
labels, assignees, and milestone. Results are returned in paginated format."
```

**Right-sized** (~18 tokens):
```
...
**Before** (3 tools, ~600 tokens):
- `tavily_search(query)` — Search via Tavily
- `brave_search(query)` — Search via Brave
- `google_search(query)` — Search via Google

**After** (1 tool, ~200 tokens):
- `web_search(query, provider="tavily"|"brave"|"google")` — Search the web
...
## 8. Tool Development Best Practices
...
## 17. Gateway Compatibility

If your MCP server will be deployed behind a gateway (Docker MCP Gateway, Composio, Bifrost, etc.), follow these additional guidelines.

See [Gateway Patterns Guide](./gateway_patterns.md) for comprehensive architecture details.

### Catalog Metadata

Provide rich metadata so gateways can index and discover your server:
- Clear, keyword-rich server name and description
- Category tags (e.g., "communication", "development", "analytics")
- Version information for compatibility tracking

### Health Check Endpoint

Gateways manage server lifecycle (start/stop containers). Provide a health check:
- Respond to health probes within 5 seconds
- Return server status, version, and available tool count
- Handle graceful shutdown signals (SIGTERM)

### Stateless Design

Gateways may start and stop your server container between requests:
- Don't rely on in-memory state between tool calls
- Use external storage (Redis, database) for persistent state
- Design tools to be independently callable (no required call ordering)

### Tool Namespacing

In multi-server environments, tool name collisions are common:
- **Always prefix** tool names with your service name: `github_search`, not `search`
- Avoid generic names: `get_status`, `send_message`, `fetch_data`
- Use consistent prefix patterns across all tools in your server

### Credential Injection

Gateways inject credentials via environment variables at container startup:
- Read API keys from environment variables, never from config files
- Support standard env var names: `{SERVICE}_API_KEY`, `{SERVICE}_TOKEN`
- Validate credentials on startup and fail fast with clear error messages

### Payment and Metering Hooks

If your server is used in a paid context, support metering:
- Emit usage events (tool name, tokens consumed, latency) per call
- Support webhook callbacks for billing systems
- See [Payment & Metering Guide](./payment_metering.md) for implementation details
